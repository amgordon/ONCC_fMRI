...
function theData = CP_Localizer(thePath,listName,sName, sNum, S,EncBlock, startTrial)
...
if EncBlock == 1
    ...
    ins_txt{1} =  sprintf('In this phase, you will see a series of pictures presented on the screen.  Sometimes a single picture will be presented in the center of the screen. Sometimes an array of pictures will be presented outside the center of the screen.\n \n Please pay attention to both the single pictures and the picture arrays. Also, please keep your eyes focused on the fixation cross in the center of the sceen for both types of images.');
...
for Trial = 1:listLength
    ...
    elseif theData.cond{Trial}==2
        Screen('DrawTexture', S.Window, picPtrs(Trial), [], S.hex{1});
        Screen('DrawTexture', S.Window, picPtrs(Trial), [], S.hex{2});
        Screen('DrawTexture', S.Window, picPtrs(Trial), [], S.hex{3});
        Screen('DrawTexture', S.Window, picPtrs(Trial), [], S.hex{4});
        Screen('DrawTexture', S.Window, picPtrs(Trial), [], S.hex{5});
        Screen('DrawTexture', S.Window, picPtrs(Trial), [], S.hex{6});
        DrawFormattedText(S.Window,'+','center','center',S.textColor);
        Screen(S.Window,'Flip');
        [keys, RT] = AG3recordKeys(ons_start,goTime,S.boxNum);
